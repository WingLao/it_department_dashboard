...
```bash
cd "/Users/laoiongtek/Desktop/CDSJ5/資訊科技科組長/it_department_dashboard"
npm install
...
無限符號的粒子公式、單一等比例縮放及金色粒子設定取自本機 P5.js 原型，已整理至 `sketch.js`，不加入 SVG、圖片或其他顏色。全畫面滑鼠效果只使用金色線條軌跡；透明 canvas 不接收點擊，因此不影響 Dashboard 操作。
